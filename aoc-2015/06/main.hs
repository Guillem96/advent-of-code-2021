...
positionRange (Position2D x1 y1) (Position2D x2 y2) = [Position2D x y | x <- [x1 .. x2 - 1], y <- [y1 .. y2 - 1]]
...
applySingle ls (Just (Instruction On p1 p2)) =
  ls `Set.union` toOpen where toOpen = Set.fromList (positionRange p1 p2)
applySingle ls (Just (Instruction Off p1 p2)) =
  ls `Set.difference` toOff where toOff = Set.fromList (positionRange p1 p2)
...
applySingle' :: Map.Map Position Int -> Maybe Instruction -> Map.Map Position Int
...
removePrefix p ss
  | startsWith' p ss = drop (length p) ss
  | otherwise = ss

instructionAction :: [Char] -> Action
instructionAction s
  | startsWith' "turn on" s = On
...
instructionsCoords s
  | startsWith' "turn on" s = parsePos $ splitOn "through " $ removePrefix "turn on" s
  | startsWith' "toggle" s = parsePos $ splitOn "through " $ removePrefix "toggle" s
  | startsWith' "turn off" s = parsePos $ splitOn "through " $ removePrefix "turn off" s
...
learnCompressor is = fromCoords ps
  where
    ps = concat [[p1, p2 `add` 1] | (Just (Instruction _ p1 p2)) <- is]

compressInstructions :: [Maybe Instruction] -> (CoordinateCompression2D, [Maybe Instruction])
compressInstructions is = (cc, cis)
  where
    cc = learnCompressor is
    cis = map compressIns is
    compressIns Nothing = Nothing
    compressIns (Just (Instruction a p1 p2)) = Just (Instruction a (compress cc p1) (compress cc (p2 `add` 1)))
...
decompressArea2 cc = foldl f 0 where f acc (p, w) = acc + (decompressArea' cc p * w)
